# script for sending email response
import os
import json
...
def emailresponse(emailaddress, outurl, runerror=False):
    ...
    password = edata[
        "password"
    ]  # will need to sort out reading this from a file that is not in the repo
...
"""
    ...
    if runerror:
        message = (
            "Unfortunately there was an error in running your MCMC. Please see "
            f"{link} for more information on the error that occurred."
        )
    else:
        message = f"Your data has been analysed. The results can be found at {link}."
